...
namespace Kassiopeia
{
    ...
    class KSCyclicIterator
    {
        public:
            ...
            Bool_t operator++(); // cyclically increments the current iterator position, returning kFALSE if it hits the marked position
            Bool_t operator--(); // cyclically decrements the current iterator position, returning kFALSE if it hits the marked position
    ...
    template< class XType >
    inline KSCyclicIterator< XType >::KSCyclicIterator() :
        fBlock(0),
        fSize(0),
        fMarkedIndex(0),
        fCurrentIndex(0)
    ...
    template< class XType >
    inline void KSCyclicIterator< XType >::SetArray(XType* const block, const UInt_t size)
    {
        fBlock = block;
        fSize = size;
        fMarkedIndex = 0;            
    ...
    template< class XType >
    inline XType& KSCyclicIterator< XType >::operator*()
    {
        return fBlock[fCurrentIndex];
    }
    
    template< class XType >
    inline void KSCyclicIterator< XType >::ToMark()
    {
        fCurrentIndex = fMarkedIndex;
        return;
    }
    template< class XType >
    inline void KSCyclicIterator< XType >::IncrementMark( const Int_t pos )
    {
    ...
    template< class XType >
    inline void KSCyclicIterator< XType >::IncrementPosition( const Int_t pos )
    {
    ...
    template< class XType >
    void KSCyclicIterator< XType >::operator+()
    {
        fMarkedIndex++;
    ...
    template< class XType >
    Bool_t KSCyclicIterator< XType >::operator++()
    {
        fCurrentIndex++;
        if( fCurrentIndex == fSize )
        {
